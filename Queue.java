...
public class Queue implements QueueInterface
{
	...
	*/
	public boolean isEmpty()
	{
		...
		if(first == null) return true;  // Check if the first element is null, indicating an empty queue
	...
	*/
	public Object front() throws EmptyQueueException
	{
		...
		else
		{
			...
			return first.getData();         // Return the data of the first node in the queue
		}
	...
	*/
	public void remove() throws EmptyQueueException
	{
		...
		else
		{
			...
			first = first.getNext();        // Remove the first node by updating the first pointer to the next node
		}
	...
	*/
	...
	{
		Node n = new Node(o);           // Create a new node with the given object
		
	   	trace("add: add starts");
		

		if(first == null)               // If the queue is empty, set the new node as the first element
		{
			first = n;
		}else{
			Node c = first;             // Traverse to the end of the queue

			while(c.getNext()!=null) c = c.getNext();

			c.setNext(n);               // Link the last node to the new node
		}
