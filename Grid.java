...
public class Grid implements GridInterface, Cloneable, Comparable
{
	...
	*/
	public Grid(int d)
	{
		...
		dimension = d; // Set grid dimension
		initialiseGrid(); // Initialize grid
	...
	*/
	public Object clone()
	{
		...
		b = new Grid(dimension); // Clone grid with same dimension
		for (r = 1; r <= dimension; r++)
		{
			for (c = 1; c <= dimension; c++)
			{
				b.board[r - 1][c - 1] = (Square) this.board[r - 1][c - 1].clone(); // Clone each square
			}
		}
		b.value = this.value; // Copy value
	...
	*/
	public void setDimension(int d)
	{
		...
		this.dimension = d; // Set grid dimension
	...
	*/
	public int getDimension()
	{
		...
		return dimension; // Return grid dimension
	}
	...
	*/
	public double getWorth()
	{
		...
		return (double) this.value; // Return worth as double
	}
	...
	*/
	public void occupySquare(Location l, Symbol s)
	{
		...
		int r = l.getRow();
		int c = l.getColumn();
		this.board[r - 1][c - 1].setSymbol(s); // Occupy square at specified location
	...
	*/
	public boolean squareOccupied(Location l)
	{
		...
		return !this.board[r - 1][c - 1].isEmpty(); // Return true if square is occupied
	}
	...
	*/
	public Symbol getSymbol(Location l) 
	{
		...
		// Retrieve the symbol at the specified location on the board.
		return this.board[r-1][c-1].getSymbol();
	}
	...
	*/
	public boolean validMove(Location l)
	{
		...
		// Check if the given location is within the bounds of the board.
		if(r<1 || r>dimension || c<1 || c>dimension) return false;
		return true;
	}
	...
	*/
	public boolean gameOver()
	{
		...
		// Determine if the game has ended either by a win or a draw.
		if(!win().equals(empty))
		{
			res = true;
		} 
	...
	*/
	public boolean equals(Grid g)
	{
		...
		// Compare dimensions and value for equality.
		if(this.dimension != g.dimension) res = false;
		if(this.value != g.value) res = false;
	
		if(!res) return false;
	
		// Compare each cell symbol for equality.
		for(int i=0;i<this.dimension;i++)
		{
			for(int j=0;j<this.dimension;j++)
			{
				if(!(this.board[i][j].getSymbol().equals(g.board[i][j].getSymbol()))) res = false;
			}
		}
