...
public class GameTree implements GameTreeInterface
{
	...
	*/
	public Object getData() throws EmptyGameTreeException
	{
		...
		return root.getData(); // Return data from root
	} 
	...
	*/
	public boolean getRandomness()
	{
		...
		return moveRandomly; // Return if randomness is allowed
	}
	...
	*/
	public boolean getSymmetries()
	{
		...
		return symmetryAllowed; // Return if symmetries are allowed
	}
	...
	*/
	public int getLevel() throws EmptyGameTreeException
	{
		...
		return root.getLevel(); // Return the level of the root
	}
	...
	*/
	public GameTree getSibling() throws EmptyGameTreeException
	{
		...
		r = new GameTree(); // Create a new GameTree for sibling
		r.root = this.root.getSibling(); // Set sibling root
	...
	*/
	public void setData(Object o) throws EmptyGameTreeException
	{
		...
		root.setData(o); // Set data in root
	...
	*/
	public void setRandomness(boolean r)
	{
		...
		moveRandomly = r; // Update randomness setting
	...
	*/
	public void setSymmetries(boolean s)
	{
		...
		symmetryAllowed = s; // Update symmetries setting
	...
	*/
	public void setLevel(int l) throws EmptyGameTreeException
	{
		...
		root.setLevel(l); // Set level in root
	...
	*/
	public void setSibling(GameTree t) throws EmptyGameTreeException
	{
		...
		this.root.setSibling(t.root); // Set sibling for root
	...
	*/
	public void generateLevelDF(Stack s, Player curr) 
	{
		...
		// Determine the player for the next move based on current level
		Player nextPlayer = ((this.getLevel() + 1) % 2 == 0) ? curr : curr.opponent();
		Grid currentGrid = (Grid) this.getData();

		// Generate possible moves in a depth-first manner and add them to the stack.
		for (int row = 0; row < currentGrid.dimension; row++) 
		{
			for (int col = 0; col < currentGrid.dimension; col++) 
			{
				// Check if the current cell is empty to make a move
				if (currentGrid.board[row][col].isEmpty()) 
				{
					// Clone the current grid to create a new game state
					Grid newGrid = (Grid) currentGrid.clone();
					if (newGrid == null) 
					{
						trace("generateLevelDF: Error cloning grid, newGrid is null");
						return;
					}

					// Place the symbol of the next player on the new grid
					newGrid.board[row][col].setSymbol(nextPlayer.getSymbol());

					// Create a new tree node for this possible move
					GameTree newTreeNode = new GameTree(newGrid, this.getLevel() + 1);

					// If this is the first child, set it directly; otherwise, link as a sibling
					if (this.getChild() == null || this.getChild().isEmpty()) 
					{
						this.setChild(newTreeNode);
					} 
					else 
					{
						GameTree sibling = this.getChild();
						// Traverse the list of siblings to find the last one
						while (sibling.getSibling() != null && !sibling.getSibling().isEmpty()) 
						{
							sibling = sibling.getSibling();
						}
						// Attach the new node as the last sibling
						sibling.setSibling(newTreeNode);
					}

					// Push the new tree node onto the stack for depth-first exploration
					s.push(newTreeNode);
				}
			}
		}
	...
	*/
	public void buildGameDF(Stack s, Player curr, int d) 
	{
		...
		// Build game tree up to the given depth using depth-first search.
		while (!s.isEmpty() && this.getLevel() < d) 
		{
			// Retrieve the top node of the stack for processing
			GameTree currentTree = (GameTree) s.top();
			s.pop();

			// Generate children only if there are none for the current node
			if (currentTree.getChild() == null || currentTree.getChild().isEmpty()) 
			{
				currentTree.generateLevelDF(s, curr);
			}
		}
	...
	*/
	public void generateLevelBF(Queue q, Player curr) 
	{
		...
		// Determine the player for the next move based on the current level
		Player nextPlayer = ((this.getLevel() + 1) % 2 == 0) ? curr : curr.opponent();
		Grid currentGrid = (Grid) this.getData();

		// Generate possible moves in a breadth-first manner and add them to the queue.
		for (int row = 0; row < currentGrid.dimension; row++) 
		{
			for (int col = 0; col < currentGrid.dimension; col++) 
			{
				...
				if (currentGrid.board[row][col].isEmpty()) 
				{
					...
					// Add the new tree node to the queue for breadth-first exploration
					q.add(newTreeNode);
				}
			}
		}
	...
	*/
	public void buildGameBF(Queue q, Player curr, int d) 
	{
		...
		// add current GameTree to queue
		q.add(this);

		// Build game tree up to the given depth using breadth-first search.
		while (!q.isEmpty() && this.getLevel() < d) 
		{
			// Retrieve the front node of the queue for processing
			GameTree currentTree = (GameTree) q.front();
			q.remove();

			// Generate children only if there are none for the current node
			if (currentTree.getChild() == null || currentTree.getChild().isEmpty()) 
			{
				currentTree.generateLevelBF(q, curr);
			}
		}
